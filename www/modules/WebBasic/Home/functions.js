...
$(document).ready(function() {
	...
	setTimeout(function() {
	  ...
	  accordionRespond('#home-narrative', false);
	}, 3000);
	
      //CHECK FOR MOBILE    
      var ua = navigator.userAgent,
      	
      	//UPDATED BY MJP - 20140218
      	//**********************************************//
      	
        isMobile = ( $('.isMobile').css('display') === 'none' ),
        isTablet = (navigator.userAgent.match(/Android|BlackBerry|iPhone|iPad|iPod|Opera Mini|IEMobile/i)),
        
        //**********************************************//

        // /WebKit/.test(ua) && /Mobile/.test(ua),
        isIE = $('html').hasClass('lt-ie9');
	 ...
	 if (isTablet && isMobile){
      ...
      //  INIT ISCROLL & STELLAR
      $(function(){
        var iScrollInstance;

         //MAP SECTION ANIMATION
        $('#map').on('enterviewport', function() {
        ...
        if (isMobile || isIE || isTablet) {} 
        else {
  
         // STELLAR.JS INIT IF NOT MOBILE
           $(window).stellar({
               responsive: true,  // Refreshes parallax content on window load and resize
    ...
    // INIT ANIMATIONS
    var animation1 = new TimelineLite({
      paused: true,
      align: "normal"
    ...
    var animation2 = new TimelineLite({
      paused: true,
      align: "normal",
       onStart: function(){ $('#map').addClass('active'); },
      onComplete: function(){$('#more-factoids-btn').addClass('active')}
    });
    
      //ANMIATION 1
       animation1.add( TweenMax.to( "#ipad", 1.0, {opacity: 1.0}) );
       animation1.add( TweenMax.from("#ipad-container", 0.5, { top: -800, delay:.2, ease:Bounce.easeOut}) );
       animation1.add( TweenMax.from("#ipad-crack", 0.1,{ opacity: 0}) );
       animation1.add( TweenMax.to("#depot-repairs", 1.0, { opacity: 1.0, delay:.5}) );
       animation1.add( TweenMax.from("#truck-boxes-container img", 2.0,{ left: -950, delay:0}) );
       animation1.add( TweenMax.to("#in-home-repairs", 1.0, { opacity: 1.0, delay:.2}) );
       animation1.add( TweenMax.from("#sun", 1.5, { top: 50, opacity:0}) );

       // ANIMATION 2
       // animation2.add( TweenMax.to("#ipad, #depot-repairs, #in-home-repairs", 0.1, {opacity: 0.0,delay: 0 }) );
       animation2.add( TweenMax.to("#next-business-day", 1.0, { opacity: 1.0 }) );
       animation2.add( TweenMax.staggerFrom("#boxes-container img", 0.17, {scale: 1.0,opacity: 0.0,top: -100,delay: 0.1}, 0.08) );
       animation2.add( TweenMax.to("#stars", 1, {opacity: 1.0,delay: .2}) );
       animation2.add( TweenMax.staggerFrom("#stars img", 0.20, {scale: 0.0, delay: 0}, 0.05) ); 
       animation2.add( TweenMax.to("#people-helped", 1.0, { opacity: 1.0, delay: .8 }) );
       animation2.add( TweenMax.staggerFrom("#blue-man-container img", 1.2, {opacity: 0}, 0.20) );
        
    // MORE FACTOIDS BUTTON
    $('#more-factoids-btn').click(function(e){
      //if first animation is running or finished, 'click' triggers animation 2
      if( animation2.isActive() || animation2.progress() == 1 ){
         animation2.restart().pause();
         animation1.restart();
      }
      //if second animation is running or finished, 'click' triggers animation 1
      else if( animation1.isActive() || animation1.progress() == 1 )
      {
       animation1.restart().pause();
       animation2.restart(); 
      }
      ...
      //ROTATE CUSTOMER COMMENTS
     /*(function(){

  var data,
      comments = $('.customer-quote'),
      count = 0;
  
  //GET CUSTOMER COMMENTS DATA AND STORE IN DATA OBJ
  $.getJSON("/content/dam/asuriondc/docs/comments.json", function(json){
      data = json;

      $.each( comments , function( indexInArray ){
     var quote = $(this),
         quoteContainer = quote.parent('.customer-quote-container'),
         interval = 10000,
         delay = 3500,
         speed = 1000,
         obj = data[count];

     quote.find('p.quote').empty().append(obj.comment);
     quote.find('span.quote-name').empty().append(obj.name);
     quote.find('span.quote-date').empty().append(obj.date);
    ++count;
    ...
    function accordionRespond( el, init ) {
		var elem = $(el),
			winW = $(window).width(),
			elemH = elem.height(),
	        l = elem.children().length,
	        h = elem.find('h3').outerHeight();
			
			if(init == true){
				h = elem.find('h3').height();
			}

	    if( elem.find('li.active').length === 0 ){
       		elem.find('li:not(:first-child) .slide-content').hide(); //hide all but first slide
		...
		if( winW >= 768 ){
			...
			elem.find('.slide-inner, h3:before').width( winW - (h*l) ).height(elemH)
			.css({
				'transform': 'rotate(-270deg)',
		   	...
		   	elem.find('.slide-content').height( winW - (h*l) );
		}//end if
       		...
       		elem.find('.slide-inner').width('auto').height('auto')
       		.css({
				'transform': 'rotate(0deg)',
				'-moz-transform': 'rotate(0deg)',
				'-webkit-transform': 'rotate(0deg)',
		   	...
		   	elem.find('.slide-content').height('auto');
       	}//end else
       	...
       	elem.find('i').click(function(e){
       		...
       		var $this = $(this);
       		$this.closest('li').next().find('h3').trigger('click');
       		if( $this.closest('li').is(':last-child') ){
       			$this.closest('ul').find('li:first-child h3').trigger('click');
       		}
       		else{
       	...
       	//accordion action
       	elem.find('h3').click(function(e) {
       		if( $(this).next('.slide-content').is(':visible') ) {
       			// $(this).next('.slide-content').slideUp(150);
       		} 
       		else {
       			elem.find('.slide-content').slideUp(250)
       			.parent().removeClass('active');	
       			$(this).parent().addClass('active');
       			$(this).next('.slide-content').slideDown(250);
       		}
       		e.preventDefault();
       	});
    ...
    };//end accordionRespond


    //init accordion on pageload
    accordionRespond('#home-narrative', true); 
    ...
    $(window).on('resize', function(e){ 
       ...
       var $eao = {
            ...
            maxStrLength: 12550,
            doAjax: function() {
                $.ajax('//api.evocalize.com/api/opinions/v1/search?apiKey=066a859a03367eed7362f9513a72ae73&isGreatContent=true&limit=100')
                    .done(function(data) {
                        $eao.data = data;
                        $eao.test = 'REMOTE';
                        $eao.imgScrub();
                    ...
                    .fail(function() {
                        $eao.data = {
                            "nextCursor": "MzEyMTU5NzY3MDQwNTY4NjQwNTo1MDox",
                            "opinionStats": {
                                "averageRating": 4.9,
                                "opinionCount": 440,
                            ...
                            "opinions": [{
                                "opinionId": "646477",
                                "submissionDate": "2014-11-07T17:40:26.000Z",
                                "submissionDateEpochMillis": 1415382026000,
                                "customer": {
                                    "name": "Julia G",
                                    "imageUrl": "https://graph.facebook.com/1573656592/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Mother lost her phone while in Atlantic City, when i was finally notified Asurion took immediate action. Phone was out of stock, website instructed me to call and agent provided excellent response."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "645997",
                                "submissionDate": "2014-11-06T02:24:08.000Z",
                                "submissionDateEpochMillis": 1415240648000,
                                "customer": {
                                    "name": "Stacey L",
                                    "imageUrl": "https://graph.facebook.com/649036650/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone was damaged and replaced the next day"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "645884",
                                "submissionDate": "2014-11-05T21:55:28.000Z",
                                "submissionDateEpochMillis": 1415224528000,
                                "customer": {
                                    "name": "Roger N",
                                    "imageUrl": "https://graph.facebook.com/1339332129/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Your response to my lost phone was fast. The service rep I spoke to was extremely helpful and understanding. The whole experience with your company was Great ! Thank You."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "644858",
                                "submissionDate": "2014-11-02T01:30:15.000Z",
                                "submissionDateEpochMillis": 1414891815000,
                                "customer": {
                                    "name": "Kari K",
                                    "imageUrl": "https://graph.facebook.com/501087320/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 3,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My daughter's phone was completely frozen and we couldn't get past the start screen.  My daughter goes to college, 30 minutes drive each direction, is a full time 911 dispatcher, and lives in an apartment by herself.   She cannot be without a phone.  I called in the evening, somewhere around 10 pm, but I didn't think there was any way they would do anything that night.  The girl was exceptionally helpful and the replacement phone was at my house by early afternoon the next day.  I can't tell you how grateful I am.  As any mom, I worry about my daughter all the time and I was extremely stressed with her not having a phone available in her circumstances."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "644294",
                                "submissionDate": "2014-10-31T13:53:19.000Z",
                                "submissionDateEpochMillis": 1414763599000,
                                "customer": {
                                    "name": "John T",
                                    "imageUrl": "https://graph.facebook.com/1121910957/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 3,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I dropped  my phone and shattered the LED. I was unable to use the phone. My shipping address had changed so I had to wait 24 hours to verify my new address. This put me around Friday night. I had trouble processing my payment and I was told I wouldn't have my new phone until Monday . Once I explained how desperately I needed the phone because I was going out of town I was transferred to  a help center and after reviewing the order the woman who was helping me understood my problem,  gave me a free upgrade, (with all the accessories) and shipped it so that I received it the next morning!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "644214",
                                "submissionDate": "2014-10-31T05:35:26.000Z",
                                "submissionDateEpochMillis": 1414733726000,
                                "customer": {
                                    "name": "Mandk U",
                                    "imageUrl": "https://graph.facebook.com/1438747177/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Dropped out of my pocket and split in two \r\n\r\nThey replaced it with the newer version, nice, and did it in 24 hours\r\n  It only cost the price of my contract. It would have been more at a store to replace.\r\n\r\nWhen the new phone would not charge again they replaced it in 24 hours, nice to have insurance."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "643648",
                                "submissionDate": "2014-10-29T20:30:46.000Z",
                                "submissionDateEpochMillis": 1414614646000,
                                "customer": {
                                    "name": "Nicholas S",
                                    "imageUrl": "https://graph.facebook.com/100007536521954/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone got water damage from a pool when the zip lock bag it was in was torn open.  The replacement phone arrived quickly and luckily my SD card was not damaged and is now functioning in my replacement phone."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "643407",
                                "submissionDate": "2014-10-29T01:41:03.000Z",
                                "submissionDateEpochMillis": 1414546863000,
                                "customer": {
                                    "name": "Julie G",
                                    "imageUrl": "https://graph.facebook.com/626251610/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My son dropped his phone in orange juice and is several thousand miles away.  Not having a phone was stressful for him.  Asurion got him a replacement in 24 hours."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "642667",
                                "submissionDate": "2014-10-26T16:05:34.000Z",
                                "submissionDateEpochMillis": 1414339534000,
                                "customer": {
                                    "name": "Laura B",
                                    "imageUrl": "https://graph.facebook.com/1179004377/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I damaged my phone at 9pm one night. Then I filled out the online form with Asurion and I had my new writing phone by 11am the next morning. Very impressive!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "642600",
                                "submissionDate": "2014-10-26T13:56:06.000Z",
                                "submissionDateEpochMillis": 1414331766000,
                                "customer": {
                                    "name": "Bruce P",
                                    "imageUrl": "https://graph.facebook.com/1493545424/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I dropped my phone on the screen cracking it much like a rock in a windshield. Asurion sent my replacement no questions. My first happy experience with an insurance company. \r\nYea!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "642583",
                                "submissionDate": "2014-10-26T13:12:49.000Z",
                                "submissionDateEpochMillis": 1414329169000,
                                "customer": {
                                    "name": "Woody H",
                                    "imageUrl": "https://graph.facebook.com/100000749565243/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "It actually was my wife's phone on our family plan with Verizon. Shenrescues dogs and she was cleaning the kennel area which has a concrete floor. She always places her phone on the cabinet top. Its at least 4 and a half' high. One of our dogs is a Hound (big dog) and was out into the common area. For some reason he stood up, put his front paws on the edge of the cabinet and \"accidentally\" we hope, knocked the S3 off the cabinet. It hit the floor really hard. End of the story...and the phone. She has her life and all the records of the animals on that phone. She freaked out, crying, etc, etc, etc.. I said \"not to worry\" we have insurance. Yes, she said, but it will take a week to get a new one back. Long story short, she got the new phone back from you guys the next day!! I had one happy wife!! \r\nThank you Asurion!!\r\nWoody Hayes"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "641937",
                                "submissionDate": "2014-10-24T19:10:10.000Z",
                                "submissionDateEpochMillis": 1414177810000,
                                "customer": {
                                    "name": "Sae L",
                                    "imageUrl": "https://graph.facebook.com/4928321/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "After receiving an email from one of our employees that dropped his phone, I called Asurion and they sent a replacement phone the next day."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "641645",
                                "submissionDate": "2014-10-24T14:11:09.000Z",
                                "submissionDateEpochMillis": 1414159869000,
                                "customer": {
                                    "name": "Stacy R",
                                    "imageUrl": "https://graph.facebook.com/1254487277/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone was dropped and the screen cracked.  I had a new phone in two days."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "641500",
                                "submissionDate": "2014-10-23T20:21:43.000Z",
                                "submissionDateEpochMillis": 1414095703000,
                                "customer": {
                                    "name": "Ginger T",
                                    "imageUrl": "https://graph.facebook.com/1002678615/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone got wet and was replaced the next day.  The process could not have been easier.  Thanks!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "640741",
                                "submissionDate": "2014-10-21T17:39:59.000Z",
                                "submissionDateEpochMillis": 1413913199000,
                                "customer": {
                                    "name": "Clint M",
                                    "imageUrl": "https://graph.facebook.com/100005841156918/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My son in law had dropped his phone at work while he was out of town, he had no other mends of communication with my daughter nor their 3 children. So within the following 10 he's of the claim he was back talking to them all. Thank you asurion, what amazing coustmer service, speed , and reliable you are."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "639997",
                                "submissionDate": "2014-10-19T04:44:24.000Z",
                                "submissionDateEpochMillis": 1413693864000,
                                "customer": {
                                    "name": "Darlene T",
                                    "imageUrl": "https://graph.facebook.com/1176956357/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My grandsons locker was broken into at junior high school and was stolen. I figured that was that & I would be stuck paying for it for the remainder of the contract ( but without the phone). Only assuriion did save the day and the theft was covered. This was the first time I actually had to use my insurance & the experience was great !! And stress free.  Thank you"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "639421",
                                "submissionDate": "2014-10-17T14:02:40.000Z",
                                "submissionDateEpochMillis": 1413554560000,
                                "customer": {
                                    "name": "Tammy P",
                                    "imageUrl": "https://graph.facebook.com/100007875375782/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My husband lost his phone while on the job site.  Asurion replaced his phone within 15 hrs.  The following day he found his phone!  Asurion gladly accepted the replacement phone back.  Couldn't have been any easier"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "639406",
                                "submissionDate": "2014-10-17T13:22:17.000Z",
                                "submissionDateEpochMillis": 1413552137000,
                                "customer": {
                                    "name": "Chris G",
                                    "imageUrl": "https://graph.facebook.com/768944716/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My son dropped his phone by a door shutting on him and I called as soon as I returned home and I had a new phone by 10:00 a.m. the next day."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "638758",
                                "submissionDate": "2014-10-15T19:45:12.000Z",
                                "submissionDateEpochMillis": 1413402312000,
                                "customer": {
                                    "name": "Jan A",
                                    "imageUrl": "https://graph.facebook.com/1309366536/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "On Sunday, my daughter dropped her phone and it landed just right on the corner to shatter the screen.  Asurion had a new phone to us by Tuesday."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "638677",
                                "submissionDate": "2014-10-15T15:48:38.000Z",
                                "submissionDateEpochMillis": 1413388118000,
                                "customer": {
                                    "name": "Natalie F",
                                    "imageUrl": "https://graph.facebook.com/1114218241/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My daughter dropped my phone, it broke and asurion replaced it the next day!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "638048",
                                "submissionDate": "2014-10-14T14:00:49.000Z",
                                "submissionDateEpochMillis": 1413295249000,
                                "customer": {
                                    "name": "Dan B",
                                    "imageUrl": "https://graph.facebook.com/100001406504204/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I broke my screen late in the day, and it was 10pm before i could get to a pc and start a claim.  The process was easy online, and the phone was sent next day air.  Naturally, with it being 10 at night on a wednesday i didn't plan to see the phone until Friday.  To my surprise, it was received at my office by noon Thursday!  What great service!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "636771",
                                "submissionDate": "2014-10-11T21:08:38.000Z",
                                "submissionDateEpochMillis": 1413061718000,
                                "customer": {
                                    "name": "Thomas H",
                                    "imageUrl": "https://graph.facebook.com/11203169/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I dropped it in the rain, broke it. And called your company and was able to get a refurbished phone in 24 hours. I wish that it would've been a new device though. It's slow with some things compared to my new iPhone that I had previously tho. But it is what it is tho I guess!!!!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "636409",
                                "submissionDate": "2014-10-11T05:25:10.000Z",
                                "submissionDateEpochMillis": 1413005110000,
                                "customer": {
                                    "name": "Taylor S",
                                    "imageUrl": "https://graph.facebook.com/100005191068387/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "The phone, my 19 year old son's lifeline, was dropped on the rocks while hiking in the mountains.  We all know that teenagers are totally lost without their phones!  Their world comes screeching to a halt if even the battery dies!  We were amazed by Asurion!  Filed the claim and POOF!!! a new replacement arrived!  Thank you very much- insuring his phone with Assurion was an excellent choice."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "634656",
                                "submissionDate": "2014-10-09T16:05:19.000Z",
                                "submissionDateEpochMillis": 1412870719000,
                                "customer": {
                                    "name": "Bob G",
                                    "imageUrl": "https://graph.facebook.com/1237056131/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "i fell in a lake,went to Verizon,the extremely helpful customer service rep checked my account for insurance info,she dialed assurion and y'all took care of my wet phone quickly.I called on a sunday,and had my new phone Tuesday morning by 10:30 am.Thank you!!!!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "631004",
                                "submissionDate": "2014-10-07T19:52:51.000Z",
                                "submissionDateEpochMillis": 1412711571000,
                                "customer": {
                                    "name": "Jerome J",
                                    "imageUrl": "https://graph.facebook.com/1545791703/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "inner screen cracked thank you for overnighting  my work tablet"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "630180",
                                "submissionDate": "2014-10-05T04:17:53.000Z",
                                "submissionDateEpochMillis": 1412482673000,
                                "customer": {
                                    "name": "Trevor R",
                                    "imageUrl": "https://graph.facebook.com/100000348339637/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 1
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "iPad was stolen without having Find My iPhone activated on the specific evening and I almost lost all my data which would have been DEVASTATING but Asurion was very professional as well as PROMPT and sent me a new iPad wishing 48 hours which meant the absolute WORLD to me at that point in time!!! THANK YOU Asurion!!!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "627356",
                                "submissionDate": "2014-09-28T21:29:56.000Z",
                                "submissionDateEpochMillis": 1411939796000,
                                "customer": {
                                    "name": "Mary G",
                                    "imageUrl": "https://graph.facebook.com/1616559286/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone was stolen.  How sad, yet Asurion had another to me the next day.  Go Asurion!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "623247",
                                "submissionDate": "2014-09-19T20:09:31.000Z",
                                "submissionDateEpochMillis": 1411157371000,
                                "customer": {
                                    "name": "Ian R",
                                    "imageUrl": "https://graph.facebook.com/1278308592/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "MY EXPERIENCE WAS A WHOLE LOT DIFFERENT THEN I EXPECTED!!!!\r\nFAST, EASY, WOW!!!  I HAVE HEARD NIGHTMARE STORIES FROM OTHERS ABOUT\r\nTHEIR INSURANCE COMPANIES, BUT I MUST SAY THIS WAS THE BEST EXPERIENCE\r\nI HAVE EVER HAD DEALING WITH INSURANCE OF ANY KIND! THANK YOU ASURION!!!!"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "621609",
                                "submissionDate": "2014-09-18T12:40:50.000Z",
                                "submissionDateEpochMillis": 1411044050000,
                                "customer": {
                                    "name": "Robert A",
                                    "imageUrl": "https://graph.facebook.com/565036751/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Phone was not charging with any charger we tried. Took it in to Verizon and confirmed it was the phone that was bad not the battery. It was late and sales person said would take two days but you had the replacement at the house the next day."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "620522",
                                "submissionDate": "2014-09-13T20:16:18.000Z",
                                "submissionDateEpochMillis": 1410639378000,
                                "customer": {
                                    "name": "Ltt M",
                                    "imageUrl": "https://graph.facebook.com/100000658852481/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My brand-new Galaxy Note 3 took an accidental dip in the pool. After drying it out and trying to get it working again over a few days, we tapped out, and turned to Asurion for a replacement.\r\n\r\nIt seemed like I had just logged out of Asurion's online claim site, and the replacement phone arrived at my door, complete with battery and charger, all good to go. It was fast. If I had known getting a replacement phone was going to be that fast and easy, I wouldn't have even bothered to try and get the damaged phone working again."
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "619435",
                                "submissionDate": "2014-09-11T02:43:21.000Z",
                                "submissionDateEpochMillis": 1410403401000,
                                "customer": {
                                    "name": "Jesus R",
                                    "imageUrl": "https://graph.facebook.com/100004723090412/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "customer service excellent got my phone back the next day"
                                    }
                                },
                                "tags": {
                                    "locale": "en-us"
                                ...
                                "opinionId": "618673",
                                "submissionDate": "2014-09-08T14:28:31.000Z",
                                "submissionDateEpochMillis": 1410186511000,
                                "customer": {
                                    "name": "Justin K",
                                    "imageUrl": "https://graph.facebook.com/510417055/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone fell out of my pocket while riding my bicycle and the screen was beyond repair! I set up a phone claim through Asurion and had a phone the next day!"
                                    }
                                },
                                "tags": {
                                    "locale": "en_US"
                                ...
                                "opinionId": "617891",
                                "submissionDate": "2014-09-06T20:41:33.000Z",
                                "submissionDateEpochMillis": 1410036093000,
                                "customer": {
                                    "name": "Randy D",
                                    "imageUrl": "https://graph.facebook.com/100002868928371/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I was amazed that I received my phone in less than 24 hours. I completed the claim around 7:30 pm and received it before 11 am the next day. Way worth it to have the insurance when your business depends on having a phone available. Thanks!"
                                    }
                                },
                                "tags": {
                                    "locale": "en_US"
                                ...
                                "opinionId": "617244",
                                "submissionDate": "2014-09-05T07:24:09.000Z",
                                "submissionDateEpochMillis": 1409901849000,
                                "customer": {
                                    "name": "Robin P",
                                    "imageUrl": "https://graph.facebook.com/100006706454043/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My husband stopped at the convenience store after working out in the heat all day. Without thinking he layed his phone down on the counter when checking out to get his wallet. He paid for his items and walked out to his truck. Suddenly he realized he did not have his phone. He went back in the store and his phone was gone. The clerk said he saw the phone on the counter but the guy behind you picked it up. My husband was devastated. I do not understand why people do things like this, but they do. I immediately stopped service on his phone and filed a claim. The phone arrived within 24 hours and he was up and running again. Thank you asurion for making it so stress free for us."
                                    }
                                },
                                "tags": {
                                    "locale": "en_US"
                                ...
                                "opinionId": "610232",
                                "submissionDate": "2014-08-26T12:10:52.000Z",
                                "submissionDateEpochMillis": 1409055052000,
                                "customer": {
                                    "name": "Jeff B",
                                    "imageUrl": "https://graph.facebook.com/100000283682724/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Phone followed my son into the pool unwillingly as it was trapped in his pocket...    Receiving a replacement the next day was a huge deal as he was scheduled to be out of town that evening, participating in a group event."
                                    }
                                },
                                "tags": []
                            }, {
                                "opinionId": "608274",
                                "submissionDate": "2014-08-22T01:01:12.000Z",
                                "submissionDateEpochMillis": 1408669272000,
                                "customer": {
                                    "name": "Bryce C",
                                    "imageUrl": "https://graph.facebook.com/1587803008/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I have my phone destroyed at work due to an equipment failure. Asurion saved me the hassle of screen repair and replaced my phone with a new or refurbished phone. Asurion quick online process and delivery ensure that I dont miss a day without my device. Thank you."
                                    }
                                },
                                "tags": []
                            }, {
                                "opinionId": "607409",
                                "submissionDate": "2014-08-20T01:10:17.000Z",
                                "submissionDateEpochMillis": 1408497017000,
                                "customer": {
                                    "name": "Joan C",
                                    "imageUrl": "https://graph.facebook.com/1345297492/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 1,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My sons phone fell in toilet, it was ruined, it was only 2 weeks old, an iPhone 5s brand new.... I had to pay 169$ but was happy we got a new one ! Asurion definitely saved mine & my sons day!"
                                    }
                                },
                                "tags": []
                            }, {
                                "opinionId": "602664",
                                "submissionDate": "2014-08-13T01:44:01.000Z",
                                "submissionDateEpochMillis": 1407894241000,
                                "customer": {
                                    "name": "Karol S",
                                    "imageUrl": "https://graph.facebook.com/1224363140/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Very quick delivery\r\nGreat replacement device\r\nGreat customer service"
                                    }
                                },
                                "tags": []
                            }, {
                                "opinionId": "610488",
                                "submissionDate": "2014-07-31T00:00:00.000Z",
                                "submissionDateEpochMillis": 1406764800000,
                                "customer": {
                                    "name": "LINDA",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I was amazed at the fast service, mailed to Asurion on Monday, they received it on Tuesday, it was fixed on Wednesday, sent back to me on Thursday, received on Friday. Now that is excellent service."
                                    }
                                },
                                "tags": {
                                    "sr_number": "78133071",
                                    "payee_name": "WARRANTY LOGISTICS 230205",
                                    "dealer_name": "TARGET CORPORATION",
                                    "product_brand": "APPLE INC",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "TABLET PC"
                                }
                            }, {
                                "opinionId": "610491",
                                "submissionDate": "2014-07-31T00:00:00.000Z",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I was anticipating frustration and incomplete service with the warranty service, but I am 100% satisfied!  The person who answered the call was easy to understand, the shipping both ways was paid for, and the problem was fixed, all within a week!  I would definitely buy another warranty."
                                    }
                                },
                                "tags": {
                                    "sr_number": "78313863",
                                    "payee_name": "WARRANTY LOGISTICS 230205",
                                    "dealer_name": "WAL-MART.COM",
                                    "product_brand": "APPLE INC",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "TABLET PC"
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "The agent I worked with was very knowledgeable and extremely helpful!"
                                    }
                                },
                                "tags": {
                                    "sr_number": "78319083",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Was very pleased with the speed of the repair and the clear communication throughout the process from start to finish."
                                    }
                                },
                                "tags": {
                                    "sr_number": "78067665",
                                    "payee_name": "WARRANTY LOGISTICS 0113914",
                                    "dealer_name": "WALMART STORES",
                                    "product_brand": "APPLE INC",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "MP3/MINIDISK"
                                }
                            }, {
                                "opinionId": "610570",
                                "submissionDate": "2014-07-31T00:00:00.000Z",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Thank you...you have always put a smile on my kids faces"
                                    }
                                },
                                "tags": {
                                    "sr_number": "78002653",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "The extended service plan offered is an excellent service that I will always utilize. The excellent service provided by Asurion has made me a life long customer. Thank you, Asurion!"
                                    }
                                },
                                "tags": {
                                    "sr_number": "78347347",
                                    "payee_name": "WARRANTY LOGISTICS 230104",
                                    "dealer_name": "WALMART STORES",
                                    "product_brand": "HEWLETT PACKARD",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Bought the Tablet Saturday, broke it on Sunday, called warranty on Monday, received shipping box an Tuesday, shipped tablet on Wednesday and had the repaired unit back by Friday..Excellent service!"
                                    }
                                },
                                "tags": {
                                    "sr_number": "78230665",
                                    "payee_name": "WARRANTY LOGISTICS 230104",
                                    "dealer_name": "WALMART STORES",
                                    "product_brand": "ASUS COMPUTER",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "COMPUTER CPU"
                                }
                            }, {
                                "opinionId": "610503",
                                "submissionDate": "2014-07-31T00:00:00.000Z",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "This was the first time I'd ever had to send anything in and I was pleasantly surprised how easy it was."
                                    }
                                },
                                "tags": {
                                    "sr_number": "77938967",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "I was especially pleased that it did not take very long once the decision was made to ship the damaged  one to you and get it back.  I really liked that I was informed that it would need parts and it still only took a little while.\r\rThank you."
                                    }
                                },
                                "tags": {
                                    "sr_number": "77966481",
                                    "payee_name": "WARRANTY LOGISTICS 230104",
                                    "dealer_name": "WALMART STORES",
                                    "product_brand": "TOSHIBA AMERICA INC",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "LAPTOP"
                                }
                                ...
                                "opinionId": "593771",
                                "submissionDate": "2014-07-30T21:05:51.000Z",
                                "submissionDateEpochMillis": 1406754351000,
                                "customer": {
                                    "name": "Doris C",
                                    "imageUrl": "https://graph.facebook.com/100004337859470/picture",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                        "upVotes": 0,
                                        "downVotes": 0
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "My phone was stolen and I reported to verizon the next day. Verizon gave me the information to get another phone. After telephoning asurion they listen to my problem and stated they would send another out the next day. Sure enough I did recd the phone as was stated. The service was great and on time."
                                    }
                                },
                                "tags": []
                            }, {
                                "opinionId": "610688",
                                "submissionDate": "2014-07-30T00:00:00.000Z",
                                "submissionDateEpochMillis": 1406678400000,
                                "customer": {
                                    "name": "SEAN S",
                                    "customerStats": {
                                        "opinionsWritten": 1,
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "This is the best warranty service plan I have ever purchased and would recommend for everyone to take advantage of the service plan offered . No hassle warranty you just can't beat. It's paid for itself every time I've had to use it. Hands down awesome!!!! Asurion is and always will be #1 in my opinion. Thank you for your outstanding service!!!!"
                                    }
                                },
                                "tags": {
                                    "sr_number": "77894483",
                                    "payee_name": "WARRANTY LOGISTICS 230325",
                                    "dealer_name": "BJS WHOLESALE CLUB",
                                    "product_brand": "PHILIPS",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "LCD TV"
                                }
                            }, {
                                "opinionId": "610627",
                                "submissionDate": "2014-07-30T00:00:00.000Z",
                                ...
                                "answers": {
                                    ...
                                    "opinion": {
                                        "type": "opinion",
                                        "stringValue": "Phenomenal service! Fast and accurate. Love the fact they took the extra step after the repair to clean and test my camera!!!! Thank you so much"
                                    }
                                },
                                "tags": {
                                    "sr_number": "78311071",
                                    "payee_name": "WARRANTY LOGISTICS 230400",
                                    "dealer_name": "DELL INC",
                                    "product_brand": "SONY",
                                    "locale": "en_US",
                                    "survey_type": "DEPOT",
                                    "product_category": "DIGITAL CAMERA"
                                }
                            }],
                            "pageNumber": 1,
                            "totalPages": 9
            ...
            rotate: function(quote) {
                var obj = $eao.dataClean.opinions[$eao.count],
                    formattedvalue = $.datepicker.formatDate("M d, yy", $.datepicker.parseDate("yy-mm-dd", obj.submissionDate.substring(0, 10))),
                    quoteStr = '';

                if (obj.answers.rating.intValue == 5) {
                    $eao.ratingWord = 'five';
                }
                if (obj.answers.rating.intValue == 4) {
                    $eao.ratingWord = 'four';
                }
                if (obj.answers.rating.intValue == 3) {
                    $eao.ratingWord = 'three';
                }
                if (obj.answers.rating.intValue == 2) {
                    $eao.ratingWord = 'two';
                }
                if (obj.answers.rating.intValue == 1) {
                    $eao.ratingWord = 'one';
                }
                quote.find('em').attr('class', $eao.ratingWord + ' col-sm-6');

                if (typeof obj.customer.imageUrl != 'undefined') {
                    quote.find('img').attr('src', obj.customer.imageUrl);
                    quote.find('.customer-quote-content').removeClass('noImg');
                } else {
                    quote.find('.customer-quote-content').addClass('noImg');
                }

                if (obj.answers.opinion.stringValue.length > $eao.maxStrLength) {
                    quoteStr = obj.answers.opinion.stringValue.substring(0, $eao.maxStrLength) + "...";
                } else {
                    quoteStr = obj.answers.opinion.stringValue;
                }
                quote.find('p.quote').html(quoteStr);
                quote.find('p.quote-date').html(formattedvalue);
                quote.find('span.quote-name').html(obj.customer.name);
            },
            imgScrub: function() {
                var dcIdx = 0;
                for (i = 0; i < $eao.data.opinions.length; i++) {
                    var cust = $eao.data.opinions[i].customer;
                    if (typeof cust.imageUrl == 'string') {
                        $eao.dataClean.opinions[dcIdx] = $eao.data.opinions[i];
                        dcIdx++;
                    }
                };
                $eao.cleanIdx = dcIdx;
            ...
            initRotate: function() {
                //Rotate through each
                $.each($eao.comments, function(indexInArray) {
                    var quote = $(this),
                        quoteContainer = quote.parent('.customer-quote-container');
